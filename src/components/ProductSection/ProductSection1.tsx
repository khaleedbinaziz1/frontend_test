...
interface Product {
  ...
  costPrice?: string;
  regularPrice?: string;
  salePrice: string; // This is the actual selling price
  stockStatus?: string;
  stockNumber?: string;
  showProduct?: boolean;
...
const ProductSection1: React.FC = () => {
    ...
    const fetchProductsAndCategories = async () => {
      try {
        ...
        const productUrl = searchQuery 
          ? `https://swish-server.vercel.app/products?q=${encodeURIComponent(searchQuery)}`
        ...
        if (fetchedProducts.length > 0) {
          const prices = fetchedProducts.map((p: Product) => parseFloat(p.salePrice) || 0);
  ...
  const allCategories = ['All', ...categories.map(category => category.name)];

  // Find min and max prices from products
  const minPrice = products.length > 0 ? Math.min(...products.map(p => parseFloat(p.salePrice) || 0)) : 0;
  const maxPrice = products.length > 0 ? Math.max(...products.map(p => parseFloat(p.salePrice) || 0)) : 10000;
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center justify-between w-full px-4 py-3 bg-secondary border border-primary/20 hover:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all duration-200 font-medium text-primary rounded-md"
            >
            ...
            {categoryDropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-secondary border border-primary/20 shadow-lg z-20 max-h-60 overflow-y-auto rounded-md">
                {allCategories.map((category) => (
                  <button
                    key={category}
                    onClick={() => handleCategorySelect(category)}
                    className={`w-full px-4 py-3 text-left hover:bg-primary/5 transition-colors text-sm font-medium ${
                      selectedCategory === category ? 'bg-primary text-secondary hover:bg-primary/90' : 'text-primary'
                    }`}
                  >
            ...
            {currentItems.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {currentItems.map((product) => (
                  <motion.div
                    key={product._id}
                    className="group relative bg-secondary border border-primary/20 overflow-hidden hover:border-primary transition-all duration-300 rounded-md shadow-sm hover:shadow-md"
  ...
  1
                          ...
                          className="w-full bg-secondary flex items-center justify-center p-1.5 md:p-2 hover:bg-primary hover:text-secondary transition-all duration-300 border border-primary/30 hover:border-primary text-xs md:text-sm font-medium rounded disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Add to Cart"
                        >
  ...
  Buy Now
        ...
        {filteredProducts.length > 0 && totalPages > 1 && (
          <div className="mt-8 flex justify-center">
            <nav className="relative z-0 inline-flex border border-primary/20 rounded-md overflow-hidden" aria-label="Pagination">
              <button
                onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                disabled={currentPage === 1}
                className="relative inline-flex items-center px-2 md:px-4 py-2 border-r border-primary/20 bg-secondary text-sm font-medium text-primary hover:bg-primary/5 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                  ...
                  <button
                    ...
                    className={`relative inline-flex items-center px-2 md:px-4 py-2 text-sm font-medium ${
                      currentPage === page
                        ...
                        : 'bg-secondary border-r border-primary/20 text-primary hover:bg-primary/5'
                    }`}
                  >
                    {page}
                  </button>
              ...
              <button
                ...
                className="relative inline-flex items-center px-2 md:px-4 py-2 bg-secondary text-sm font-medium text-primary hover:bg-primary/5 disabled:opacity-50 disabled:cursor-not-allowed"
              >
