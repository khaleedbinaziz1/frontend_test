...
const ProductSection1: React.FC = () => {
  ...
  // For the all-products section, filter by globalCategoryFilter
  const allProductsFiltered = products.filter((product) => {
    if (globalCategoryFilter === "All") return true;
    const categoryName = categoryMap[product.category];
    return (
      categoryName &&
      categoryName.toLowerCase() === globalCategoryFilter.toLowerCase()
    );
  });

  // Handler for clicking a category banner or name
            ...
            <div ref={allProductsRef} className="mt-16">
              ...
              <div className="flex flex-col gap-4 mb-6">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="text-sm">{globalCategoryFilter.toUpperCase()}</span>
                    <ChevronDown
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-secondary border border-primary/20 shadow-lg z-20 max-h-60 overflow-y-auto rounded-md">
                      {["All", ...categories.map((category) => category.name)].map((category) => (
                        <button
                          key={category}
                          onClick={() => {
                            setGlobalCategoryFilter(category);
                            setCategoryDropdownOpen(false);
                          }}
              ...
              {allProductsFiltered.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                  {allProductsFiltered.slice(indexOfFirstItem, indexOfLastItem).map((product) => (
                    <motion.div
                      key={product._id}
                      className="group relative bg-secondary border border-primary/20 overflow-hidden hover:border-primary transition-all duration-300 rounded-md shadow-sm hover:shadow-md"
                      initial={{ opacity: 0, y: 20 }}
